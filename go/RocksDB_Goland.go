...
// #cgo LDFLAGS: -L./../rocksdb -lrocksdb -lz -lpthread -lsnappy -lstdc++ -lm -ldl -O3
import "C"
import (
	"errors"
	"regexp"
...
func newRocksdb(path string) *rocksdb {
	...
	var e *C.char
	db := C.rocksdb_open(options, C.CString(path), &e)
	if e != nil {
		panic(C.GoString(e))
	}
	C.rocksdb_options_destroy(options)
	return &rocksdb{db, C.rocksdb_readoptions_create(), C.rocksdb_writeoptions_create(), path, e}
}

func (c *rocksdb) CloseRocksdb() {
	C.rocksdb_writeoptions_destroy(c.wo)
	C.rocksdb_readoptions_destroy(c.ro)
	//rocksdb_options_destroy(options);
	C.rocksdb_close(c.db)
}
...
func (c *rocksdb) Del(key string) error {
	k := C.CString(key)
	defer C.free(unsafe.Pointer(k))
